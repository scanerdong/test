...
#define DEFAULT_ENTRIES_NUM (1024*4)
#define DEFAULT_BUCKETS_NUM (1024)

__SHM_STL_BEGIN

template <typename _Key, typename _Value>
struct Assignment {
    void operator() (volatile shm_stl::Node<_Key, _Value > &node, const _Value &new_value, bool& endloop) {
        node.m_value = new_value;
    }
};
...
template <typename _Key, typename _Value>
struct GetPnode {
    void operator() (shm_stl::Node<_Key, _Value > &node, shm_stl::Node<_Key, _Value >* &pnode, bool& endloop) {
        pnode = &node;
		endloop = true;
    }
};
...
struct DeleteElement {
    void operator() (volatile shm_stl::Node<_Key, _Value > &node, int& not_used, bool& endloop) {
        delete node.m_value;
    }
	
    void operator() (volatile shm_stl::Node<_Key, _Value > &node, int& not_used, bool& needremove, bool& endloop) {
        delete node.m_value;
		needremove = true;
    }
};
