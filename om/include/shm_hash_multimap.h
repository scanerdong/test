...
#ifndef __SHM_HASH_MULTIMAP_H_
#define __SHM_HASH_MULTIMAP_H_

#include "shm_hash_multitable.h"
#include "shm_profiler.h"
#include "management_plane.h"
#include <iostream>
#include <sstream>
#include <fstream>
#include <sys/types.h>
...
class hash_multimap {
    public:
        ...
        typedef hash_multitable<key_type, value_type, hasher, key_equal> _Ht;

    public:
		//if dynamic_growth == true; entries is not used.
        hash_multimap(const char * name, bool dynamic_growth = true, uint32 entries = DEFAULT_ENTRIES_NUM, uint32 buckets = DEFAULT_BUCKETS_NUM, uint32 bucket_age_num=0)
			: m_entries(entries), m_buckets(buckets), m_dynamic_growth(dynamic_growth),m_bucket_age_num(bucket_age_num) {
                     snprintf(m_name, sizeof(m_name), "HT_MUL_%s", name);
					 create_or_attach();
                 }

        hash_multimap(bool dynamic_growth = true, uint32 entries = DEFAULT_ENTRIES_NUM, uint32 buckets = DEFAULT_BUCKETS_NUM, uint32 bucket_age_num=0)
			: m_entries(entries), m_buckets(buckets), m_dynamic_growth(dynamic_growth),m_bucket_age_num(bucket_age_num) {
			         m_name[0] = '\0'; 
					 create_or_attach();
	             }
        ...
        inline bool create_or_attach(void) {
            ...
            if (proc_type == RTE_PROC_PRIMARY) {
                ...
                if(m_name[0]){
                    ...
                    snprintf(m_name, sizeof(m_name), "HT_MUL_ANO_%d", rte_atomic32_add_return((rte_atomic32_t*)&global_hash_count, 1));
