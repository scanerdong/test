...
#ifndef __SHM_HASH_MULTITABLE_H_
#define __SHM_HASH_MULTITABLE_H_
...
class hash_multitable {
    public:
        ...
        typedef MultiBucket<node_type, key_type, value_type, key_equal>  bucket_type;

    public:
        hash_multitable(bool dynamic_growth = false, uint32 entries = DEFAULT_ENTRIES_NUM, uint32 buckets = DEFAULT_BUCKETS_NUM, uint32 bucket_age_num=0)
            : m_mask(0),m_entries_num(entries), m_buckets_num(buckets), m_bucket_array(NULL), m_dynamic_growth(dynamic_growth) {
                initialize();
            }

        ~hash_multitable(void) {finalize();}
