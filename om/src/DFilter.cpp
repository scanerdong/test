...
test_free(void* value)
{
	...
	if (test->val1)
		DFilter::stnode_free(test->val1);
	if (test->val2)
		DFilter::stnode_free(test->val2);
...
sttype_t* DFilter::sttype_array;
int(*DFilter::op_ref_table)[2];
...
stnode_t* DFilter::stnode_new(sttype_id_t type_id, void* data)
{
	sttype_t	*type;
	stnode_t	*node;
...
void DFilter::stnode_free(stnode_t *node)
{
	if (node->type) {
...
void DFilter::stnode_new_data(stnode_t *node, void* data)
{
...
void DFilter::stnode_free_data(stnode_t *node)
{
	if (node->type) {
		if (node->type->func_free) {
			node->type->func_free(node->data);
...
buffer_t* DFilter::trans_to_buffer(const char* data)
{
...
int DFilter::scan(const char* pchSrc)
{
    ...
    {
        //跳过前面的空格
        while(pchSrc[pos]==' ' || pchSrc[pos]=='\t') pos++;
...
int DFilter::num_operands(int op)
{
	switch(op) {
...
void
DFilter::sttype_test_set1(stnode_t *node, test_op_t op, stnode_t *val1)
{
	test_t	*test;
...
void
DFilter::sttype_test_set2(stnode_t *node, test_op_t op, stnode_t *val1, stnode_t *val2)
{
	test_t	*test;
...
void
DFilter::sttype_test_set2_args(stnode_t *node, stnode_t *val1, stnode_t *val2)
{
	test_t	*test;
...
void
DFilter::sttype_test_get(stnode_t *node, test_op_t *p_op, stnode_t **p_val1, stnode_t **p_val2)
{
	test_t	*test;
...
void DFilter::sttype_range_set(stnode_t *node, stnode_t *val1, stnode_t *val2, stnode_t *val3)
{
	range_t	*range;
    ...
    DFilter::stnode_free(val1);
    DFilter::stnode_free(val2);
    DFilter::stnode_free(val3);
}

int DFilter::make_ast()
{
...
int DFilter::semantic_check(stnode_t* node)
{
    switch(node->type->id){
...
bool DFilter::exec_msg(stnode_t* node, relay_msg_base& msg) //执行过滤
{
    switch(node->type->id){
        case STTYPE_UNINITIALIZED:
            return true; 
...
bool DFilter::is_eq(stnode_t *arg1, stnode_t *arg2, relay_msg_base& msg)
{
    switch(arg1->type->id){
        case STTYPE_FIELD:
        {
