...
void
IpDefragProcessor::atomic_sub(int ile, int *co)
{
  *co -= ile;
}

void
IpDefragProcessor::atomic_add(int ile, int *co)
{
  *co += ile;
}

void
IpDefragProcessor::kfree_skb(struct sk_buff * skb, int type)
{
  (void)type;
  free(skb);
}
...
void
IpDefragProcessor::panic(char *str)
{
...
void
IpDefragProcessor::add_timer(struct timer_list * x)
{
  if (timer_tail) {
    timer_tail->next = x;
    x->prev = timer_tail;
...
void
IpDefragProcessor::del_timer(struct timer_list * x)
{
  if (x->prev)
    x->prev->next = x->next;
  else
...
void
IpDefragProcessor::frag_kfree_skb(struct sk_buff * skb, int type)
{
  if (this_host)
...
void
IpDefragProcessor::frag_kfree_s(void *ptr, int len)
{
  if (this_host)
...
void *
IpDefragProcessor::frag_kmalloc(int size, int dummy)
{
...
struct ipfrag *
IpDefragProcessor::ip_frag_create(int offset, int end, struct sk_buff * skb, unsigned char *ptr)
{
  struct ipfrag *fp;
...
*/
struct ipq *
IpDefragProcessor::ip_find(struct ip * iph)
{
  struct ipq *qp;
  struct ipq *qplast;
...
*/
void
IpDefragProcessor::ip_free(struct ipq * qp)
{
  struct ipfrag *fp;
  struct ipfrag *xp;
...
void
IpDefragProcessor::ip_expire(unsigned long arg)
{
  struct ipq *qp;
...
*/
void
IpDefragProcessor::ip_evictor(void)
{
...
*/
struct ipq *
IpDefragProcessor::ip_create(struct ip * iph)
{
  struct ipq *qp;
  int ihlen;
...
int
IpDefragProcessor::ip_done(struct ipq * qp)
{
  struct ipfrag *fp;
  int offset;
...
*/
char *
IpDefragProcessor::ip_glue(struct ipq * qp)
{
  char *skb;
  struct ip *iph;
  struct ipfrag *fp;
  ...
  iph = (struct ip *) skb;
  iph->ip_off = 0;
  iph->ip_len = htons((iph->ip_hl * 4) + count); //重新设置IP数据包的长度
  // skb->ip_hdr = iph;
...
IpDefragProcessor::IpDefragProcessor(uint32_t u32HashSize):
    numpack(0),timer_head(NULL),timer_tail(NULL),dev_addon(16),sk_buff_size(168)
{
    struct timeval tv;
    
    gettimeofday(&tv, 0);
    time0 = tv.tv_sec;
    fragtable = (struct hostfrags **) calloc(u32HashSize, sizeof(struct hostfrags *));
    if (!fragtable)
      no_mem("IpDefragProcessor");
    hash_size = u32HashSize;
}

IpDefragProcessor::~IpDefragProcessor()
{
    if (fragtable) {
      free(fragtable);
      fragtable = NULL;
...
int IpDefragProcessor::ip_defrag_stub(struct ip *iph, struct ip **defrag)
{
...
char* IpDefragProcessor::ip_defrag(struct ip *iph, struct sk_buff *skb)
{
...
int IpDefragProcessor::hostfrag_find(struct ip * iph)
{
...
void IpDefragProcessor::hostfrag_create(struct ip * iph)
{
...
char* IpDefragProcessor::test_malloc(int x)
{
...
int IpDefragProcessor::frag_index(struct ip * iph)
{
...
void IpDefragProcessor::no_mem(char *func)
{
