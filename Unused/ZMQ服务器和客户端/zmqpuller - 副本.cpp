...
int main(int argc, char *argv[])
{
    ...
    string strTest;
    char pchTest[100] = "hahaddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd";
    string strB;
    
    strTest = pchTest;
    strB = strTest;
    printf("str:0x%08lx, pch:0x%08lx, strB:0x%08lx\n", (uint64_t)strTest.c_str(), (uint64_t)pchTest, (uint64_t)strB.c_str());

    
    
    DpiArg stDpiArgHttp;
    printf("stDpiArgHttp.nServerIp:%u, stDpiArgHttp.nServerPort:%u, stDpiArgHttp.strUri:%s, stDpiArgHttp.strHostName:%s, stDpiArgHttp.strUserAgent:%s, stDpiArgHttp.strContentType:%s\n", 
        stDpiArgHttp.nServerIp, stDpiArgHttp.nServerPort, stDpiArgHttp.strUri.c_str(), stDpiArgHttp.strHostName.c_str(), 
        stDpiArgHttp.strUserAgent.c_str(), stDpiArgHttp.strContentType.c_str());
	...
	while(1)
    {
        ...
        sleep(1);
        dpo_pkt_header* pheader = (dpo_pkt_header*)re_string;
        printf("caplen:%u\n", pheader->_caplen);
...
{
	uint32_t x;
	 inet_aton("10.42.32.65", (struct in_addr*)&x);
	printf("%d, reverse %d, %x\n", x,ntohl(x), x);
