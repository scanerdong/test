...
//New Style
int bbuip=0;
short bbu_udp_port = 33333;//通道建立请求端口
short bbu_iq_port  = 33338;//IQ数据端口
short bbu_cm_port  = 30000;//CM数据端口
int rruip=0;
...
void* rht_process(void* arg)
{
    char achBuf[1024];
    int csockfd = *(int*)arg;
    unsigned int u32RecvCount = 0;

    while(1)
    {
        memset(achBuf, 0, 1024);
        if(recv(csockfd, achBuf, 1024, 0) > 0)
        {
            u32RecvCount++;
            printf("rht_process recv cout %d recved--%s\n", u32RecvCount, achBuf);
        }
        sleep(1);
    }
}
...
void* ht_process(void* arg)
{
	...
	char buf[]={
		...
		0x00,0x01,//流水号
		0x00,0x00,0x00,0x00,0x00,0x00,0x00,//时间
		0x04,//命令字：心跳包
		0xFF,//应答标志
		0x00,0x00,0x00,0x00,//扩展字段
		0x27,0x6F//校验和
	};
	unsigned int cnt=0;
	int ret;
	...
	fprintf(stdout, "Connecting to RRU\n");
	addr_len = sizeof(struct sockaddr_in);
	while (1) {

		csockfd = accept(sockfd, (struct sockaddr *)&addr, &addr_len);
		if (csockfd<0) {
			fprintf(stderr, "RRU is offline: %d %s\n", cnt, strerror(errno));
			ret = connect_to_rru();
			if (ret<0) {
				printf("Fail retry %d...\n", ++cnt);
				sleep(1);
        ...
        int rcvbuf_len = 10 * 1024;  //ʵ�ʻ�������С��һ�롣
        int len = sizeof(rcvbuf_len);
        if(getsockopt(csockfd, SOL_SOCKET, SO_SNDBUF, (void *)&rcvbuf_len, &len) < 0 ){
            perror("getsockopt: ");
            return -1;
        }
        else
        ...
        pthread_t tid_rht;
        ret = pthread_create(&tid_rht, NULL, rht_process, (void*)&csockfd);
        if(ret != 0)
        {
...
void usage(const char *progname)
{
  ...
  fprintf(stderr, "  -u UP_CAP_CHANNEL    指定要捕获的通道号（1/2），默认是1\n");
